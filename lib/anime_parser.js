...
export const scrapeWatchAnime = async ({ id }) => {
  try {
    ...
    const gogoServerLink = $('li.vidcdn > a').attr('data-video')
    ...
    if (gogoServerLink) {
      const gogoWatch = gogoServerLink;
      ...
      const params = await generateEncryptAjaxParameters($$, gogoWatchLink.searchParams.get('id'));
      ...
      const finalSource = await decryptEncryptAjaxResponse(fetchRes.data);
      if (finalSource.source) {
        finalSource.source.map(src => sources.push(src));
      }
      if (finalSource.source_bk) {
    ...
    return {
      video: episode_link,
      gogoserver: gogoServerLink,
      gogoSourcesHLS: sources,
      gogoSourcesBackupHLS: sources_bk,
      streamsb: streamsb,
